...
contract Voter is IVoter, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _ve, address _factory, address  _gauges, address _bribes, address _governor) initializer public {
        __Ownable_init();
    ...
    function reset(uint _tokenId) external onlyNewEpoch(_tokenId) nonReentrant {
    ...
    function vote(uint tokenId, address[] calldata _poolVote, uint256[] calldata _weights) external onlyNewEpoch(tokenId) nonReentrant {
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        address _internal_bribe = IBribeFactory(bribefactory).createBribe(emergencyCouncil, tokenA, tokenB);
        address _external_bribe = IBribeFactory(bribefactory).createBribe(emergencyCouncil, tokenA, tokenB);
        address _gauge = IGaugeFactory(gaugefactory).createGauge(_pool, _internal_bribe, _external_bribe, ve, isPair, allowedRewards);
    ...
    function attachTokenToGauge(uint tokenId, address account) external {
        ...
        if (tokenId > 0) IVotingEscrow(ve).attach(tokenId);
    ...
    function detachTokenFromGauge(uint tokenId, address account) external {
        require(isGauge[msg.sender]);
        if (tokenId > 0) IVotingEscrow(ve).detach(tokenId);
    ...
    function claimBribes(address[] memory _bribes, address[][] memory _tokens, uint _tokenId) external {
        require(IVotingEscrow(ve).isApprovedOrOwner(msg.sender, _tokenId), "not owner");
