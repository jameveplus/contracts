...
contract Bribe is ReentrancyGuard {
    ...
    /* ========== CONSTRUCTOR ========== */
    constructor(address _owner, address _voter, address _bribeFactory)  {
    ...
    function balanceOfAt(uint256 _token, uint256 _timestamp) public view returns (uint256) {
        return _balances[_token][_timestamp];
    }
    // get last deposit available balance (getNextEpochStart)
    function balanceOf(uint256 _token) public view returns (uint256) {
        uint256 _timestamp = getNextEpochStart();
        return _balances[_token][_timestamp];
    }

    function rewardThisEpoch(address _token) public view returns(uint256){
        uint256 _timestamp = getEpochStart();
        return rewardData[_token][_timestamp].rewardsPerEpoch;
    }
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward) external nonReentrant {
        require(IVoter(voter).isWhitelisted(_rewardsToken), "bribe tokens must be whitelisted");
